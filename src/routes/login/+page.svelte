...
{#if showAlert}
  <div class="alert alert-info shadow-l flex justify-start">
    ...
    <p class="justify-left">Wrong username or password. Please try again.</p>
  </div>
{/if}



<h1 class="text-center text-xl mt-20 text-primary">Please Log In</h1>
...
<form on:submit={_logInUser} class="w-1/3">
    ...
    <input type="text" name="username" placeholder="whatdude" class="input input-bordered w-full" />
    </div>
    ...
    {/if}
    <button class="btn btn-md btn-primary" disabled={isLoading}>Log In</button>
    </div>
</form>
</div>
